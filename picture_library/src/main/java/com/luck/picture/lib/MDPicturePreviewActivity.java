 ...
 * @描述:图片预览
 */
public class MDPicturePreviewActivity extends PictureBaseActivity implements
        View.OnClickListener, PictureSimpleFragmentAdapter.OnCallBackActivity {
    private static final String TAG = MDPicturePreviewActivity.class.getSimpleName();
...
//            onSelectNumChange(false);
    ...
    protected void initWidgets() {
        ...
        selectedNum = getIntent().getIntExtra("selectedNum", 0);
...
//        onSelectNumChange(false);
    ...
    private void subSelectPosition() {
        for (int index = 0, len = selectData.size(); index < len; index++) {
            ...
            media.setNum(index + 1 + selectedNum);
        }
    }
...
//                }
//                tvMediaNum.setVisibility(View.VISIBLE);//民度隐藏
                mTvPictureOk.setVisibility(View.GONE);
    ...
    protected void onCheckedComplete() {
        if (adapter.getSize() > 0) {
            ...
            int currentSize = selectData.size() + selectedNum;
            ...
            if (isChecked) {
                ...
                image.setNum(selectData.size() + selectedNum);
                if (config.checkNumMode) {
