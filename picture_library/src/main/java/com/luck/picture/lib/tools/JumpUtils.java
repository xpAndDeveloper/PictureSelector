...
import com.luck.picture.lib.MDPicturePreviewActivity;
...
public class JumpUtils {
     ...
     * @param requestCode
     */
    public static void startMDPicturePreviewActivity(Context context, Bundle bundle, int requestCode) {
        if (!DoubleUtils.isFastDoubleClick()) {
            Intent intent = new Intent();
            intent.setClass(context, MDPicturePreviewActivity.class);
            intent.putExtras(bundle);
            if (!(context instanceof Activity)) {
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                context.startActivity(intent);
